...
impl App {
    async fn load_qrcode(&self) -> QrCode {
        // 获取应用实例
        let app = self.app_ui.upgrade().expect("Cannot get app object");
        // 设置登录状态为未登录
        app.global::<AppStatus>().set_logined(false);

        let qr_code = self.app_lib
            .client
            .login_qrcode()
            .compat()
            .await
        ...
        // 生成二维码
        let qr_code_path = self.app_lib.config.cache_dir.join("qrcode.png");
    ...
    async fn bind_qrcode_login(&self) {
        let qr_code = self.load_qrcode().await;
        ...
        app.global::<LoginProperty>().on_check_qrcode(move || {
            ...
            let _ = slint::spawn_local(async move {
                if let Ok(cookie_str) = app_runtime.app_lib.client.login_check_qrcode(qr_code).compat().await
                {
                    // println!("Login success, cookie:\n {}", cookie_str);
                    app_runtime.app_lib.init(&cookie_str).await.unwrap();
                    ...
                    app_runtime.bind_qrcode_login().await;
                }
            });
        });
    }
